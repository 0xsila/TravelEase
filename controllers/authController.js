...
const { generateOTP, verifyOTP, sendOTPEmail, sendActivationEmail } = require('../utils/emailService');
...
exports.getOTPVerificationData = async (req, res, next) => {
  try {
    ...
    // Find user
    const user = await User.findById(userId).select('email firstName lastName isActivated');
    if (!user) {
    ...
    if (!user.isActivated) {
      ...
      try {
        ...
        return res.status(202).json({
          success: true,
          message: 'Account not activated. We have sent a new activation code to your email.',
          userId: user._id,
