  ...
  try {
    ...
    if (!rental) {
      return res.status(404).json({
        success: false,
        message: `${rentalType.charAt(0).toUpperCase() + rentalType.slice(1)} not found`
      });
    }
    ...
    const pricePerDay = rentalType === 'car' ? rental.pricePerDay : rental.pricePerNight;
  ...
  try {
    ...
    const enhancedBookings = await Promise.all(bookings.map(async (booking) => {
      ...
      switch (booking.rentalType) {
        case 'house':
          rentalDetails = await House.findById(booking.rentalId)
            .select('title images location pricePerNight');
          break;
        case 'hotel':
          rentalDetails = await Hotel.findById(booking.rentalId)
            .select('name images location pricePerNight');
          break;
        case 'car':
          rentalDetails = await Car.findById(booking.rentalId)
            .select('make model year images location pricePerDay');
          break;
      }
